...
public function wx_http_request($url, $params, $body="", $isPost=false, $isImage=false ) {
    ...
    public function responseMsg()
    {
        ...
        $fromUsername = (string)$postObj->FromUserName;//openid
        $toUsername = (string)$postObj->ToUserName;//潘吉昌的个人测试号
        $time = time();
        ...
        switch (strtolower(trim($postObj->MsgType))) {
            case "event": // 事件
                switch (strtolower(trim($postObj->Event))) {
                    case "subscribe": // 关注事件
                        $msgType = "text";
                        $res = M("user_lu")->where("user_openid='$result->openid'")->find();
                        if ($res) {
                            if ($res['user_active'] == 0) {
                                $data['user_active'] = 1;
                                M("user_lu")->where("user_openid='$result->openid'")->save($data);
                            }
                        } else {
                            $data['user_nickname'] = $result->nickname;
                            $data['user_openid'] = $result->openid;
                            M("user_lu")->add($data);

                        }
                        $info = M("user_lu")->where("user_openid='$result->openid'")->find();
                        $urls = $this->geturl($info['id'], 1);
                        $contentStr = "欢迎关注特斯拉无线传输公众号，我们将为您带来科普知识，全球尖端科技，前沿技术，为您带来不一样的体验！   本公众号可查询天气，您只用发送你要查询的地址，公众号会自动发送给您(如果弹出输入错误，只需把地区后加入地名的级别，如新野县)。发送“笑话”可为你发送一条笑话！luzhechengtian" ;

                        /*if (isset($postObj->EventKey) && $postObj->EventKey) {. $urls
                            $arr = explode("_", $postObj->EventKey);
                            //1.用户未关注时
                            //   EventKey  事件key值，qrscene_为前缀，后面是二维码参数值
                            // 2.用户已关注时
                            //   EventKey  事件key值，是一个32位无符号整数，即创建二维码时的二维码的scene_id

                            $res1 = M("user_lu")->where("id='$arr[1]'")->find();
                            $user = M("user_lu")->where("user_openid='$result->openid'")->find();
                            if ($res1) {
                                if ($user['uesr_biaoshi'] == 0) {
                                    $data1['user_money'] = $res1["user_money"] + 1;
                                    $data2['uesr_biaoshi'] = 1;
                                    M("user_lu")->where("id = '$arr[1]'")->save($data1);
                                    M("user_lu")->where("user_openid = '$result->openid'")->save($data2);
                                }
                                $contentStr = "用户通过扫描'{$res1['user_nickname']}'的二维" . $arr[1] . "码关注tpnews" . $urls;
                            } else {
                                if ($user['uesr_biaoshi'] == 0) {
                                    $data3['uesr_biaoshi'] = 1;
                                    M("user_lu")->where("user_openid = '$result->openid'")->save($data3);
                                }
                                $contentStr = "用户通过扫描二微码关注tpnews" . $arr[1] . "用户通过扫描二微码关注tpnews" . $urls;
                            }
                        }*/


                                /*$url="人脸抽奖<a href='http://zss.zzclm.cn/index.php/Home/Face/face?openid=".$result->openid."&user=61"."&activity=62"."&game=62"."'>请点击这里</a>";*/
                        $url="开始摇一摇游戏<a href='http://lll.lkws.shop/index.php/Home/Lu/yaoyiyao'>请点击这里</a>";
                                $this->setTextMessage($fromUsername,$url);
                    ...
                    case "unsubscribe": // 取消关注事件
                        $data['user_active'] = 0;
                        $res = M("user_lu")->where("user_openid='$result->openid'")->save($data);
                        if ($res) {
            ...
            /*case "text":
                $keyword = "笑话";
                $msgType = "text";
                //$contentStr = Db::query('select * from iphone_joke where id = 1');
                //$member = M("joke");
                //$content=Db::query('select * from iphone_joke where id = 1');
                //$contentStr=Db::table('iphone_joke')->where('id',1)->select('content');
                $and=rand(0,10);
                $content=M('joke')->where("id=$and")->select();
                //$contentStr='select id from iphone_joke where id=1';
                foreach($content as $k=>$v){
                    $contentStr=$v['content'];
                }
                //$contentStr=$content[0];
           //$contentStr=Db::table("iphone_joke")->where("id",1)->select();
                $resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
			...
			$and = M('joke')->count();
                        ...
                        $contentStr="开始摇一摇游戏<a href='http://lll.lkws.shop/index.php/Home/Lu/yaoyiyao'>请点击这里</a>";
                        //$this->setTextMessage($fromUsername,$url);
                        $msgType = "text";
                        $resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
                        break;
            ...
            case "image":
                ...
                $mediaid=(string)$postObj->MediaId;
                $path="C:/code/lutp/Public/home/images/face_image/face_$openid.jpg";
                $aa=$this->downimg($mediaid,$path);
//                foreach($aa as $k=>$v){
//                    $contentStr = "$v,$path";
//                }
...
//(图片路径）↓
                            $filename = "C:/code/lutp/Public/home/images/face_image/face_$openid.jpg";
                $data = array
                            (
                ...
                /*var_dump($files['faces'][0]['attributes']);
                die;*/
                            if (!empty($files['faces'][0]['attributes'])) {
                                ...
                                /*echo "伤心程度:" . $emotion['sadness'] . "\n"
                                    . "平静程度:" . $emotion['neutral'] . "\n"
                                    . "厌恶程度:" . $emotion['surprise'] . "\n"
                                    . "高兴程度:" . $emotion['happiness'] . "\n"
                                    . "惊讶程度:" . $emotion['surprise'] . "\n"
                                    . "恐惧程度:" . $emotion['fear'] . "\n"
                                    . "恐惧程度:" . $emotion['fear'] . "\n"
                                    . "女性认为颜值程度:" . $beauty['female_score'] . "\n"
                                    . "男性认为颜值程度:" . $beauty['male_score'] . "\n"
                                    . "性别:" . $gender['value'] . "\n"
                                    . "年龄:" . $age['value'] . "\n"
                                    . "年龄:" . $age['value'] . "\n"
                                    . "健康状态:" . $skinstatus['health'] . "\n"
                                    . "青春痘:" . $skinstatus['acne'] . "\n"
                                    . "色斑:" . $skinstatus['stain'] . "\n"
                                    . "黑眼圈:" . $skinstatus['dark_circle'] . "\n"
                                    . "人种分析:" . $ethnicity['value'] . "\n";
                                unlink($facename);
                            }*/
                                ...
                                if($gender['value']=="Male"){
                                    if ($beauty['female_score'] < 40){
                                        $female_score= "女性认为颜值程度:" . "这男的好丑啊" . "\n"
                                            . "男性认为颜值程度:" . "这男的没法看，太丑" . "\n";
                                    } elseif ($beauty['female_score'] >= 40 && $beauty['female_score'] < 50) {
                                        $female_score= "女性认为颜值程度:" . "不算太丑" . "\n"
                                            . "男性认为颜值程度:" . "不算太丑" . "\n";
                                    } elseif ($beauty['female_score'] >= 50 && $beauty['female_score'] < 60) {
                                        $female_score= "女性认为颜值程度:" . "一般" . "\n"
                                            . "男性认为颜值程度:" . "一般" . "\n";
                                    } elseif ($beauty['female_score'] >= 60 && $beauty['female_score'] < 70) {
                                        $female_score= "女性认为颜值程度:" . "较帅" . "\n"
                                            . "男性认为颜值程度:" . "较帅" . "\n";
                                    } elseif ($beauty['female_score'] >= 70 && $beauty['female_score'] < 80) {
                                        $female_score= "女性认为颜值程度:" . "一个字：帅！" . "\n"
                                            . "男性认为颜值程度:" . "一个字：帅！" . "\n";
                                    } elseif ($beauty['female_score'] >= 80 && $beauty['female_score'] < 90) {
                                        $female_score= "女性认为颜值程度:" . "我的男神！" . "\n"
                                            . "男性认为颜值程度:" . "这哥们帅的我都心动了！" . "\n";
                                    } elseif ($beauty['female_score'] >= 90 && $beauty['female_score'] < 100) {
                                        $female_score="女性认为颜值程度:" . "你已经帅出了天际，无法用语言形容！吴亦凡都没你帅！" . "\n"
                                            . "男性认为颜值程度:" . "你已经帅出了天际，无法用语言形容！吴亦凡都没你帅！！" . "\n";
                                    }

                                    $contentStr= "$value"."\n"."性别:" . "男"."\n"."年龄:" . $age['value']."岁"."\n" . "健康状态:" . $skinstatus['health']."\n"."$female_score". "女性认为颜值程度:" . $beauty['female_score'] . "\n" . "男性认为颜值程度:" . $beauty['female_score']. "\n". "平静程度:" . $emotion['neutral'] . "\n"
                                        . "高兴程度:" . $emotion['happiness'] . "\n";
                                    $resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
                                }else{
                                    if ($beauty['female_score'] < 40) {
                                        $female_score= "女性认为颜值程度:" . "这女的好丑啊" . "\n"
                                            . "男性认为颜值程度:" . "这女的没法看，太丑" . "\n";
                                        ...
                                        $female_score= "女性认为颜值程度:" . "较漂亮" . "\n"
                                            . "男性认为颜值程度:" . "这女生挺漂亮的" . "\n";
                                    } elseif ($beauty['female_score'] >= 70 && $beauty['female_score'] < 80) {
                                        $female_score= "女性认为颜值程度:" . "很漂亮！" . "\n"
                                            . "男性认为颜值程度:" . "哇！快看，美女啊！！" . "\n";
                                    } elseif ($beauty['female_score'] >= 80 && $beauty['female_score'] < 90) {
                                        $female_score= "女性认为颜值程度:" . "大美女！" . "\n"
                                            . "男性认为颜值程度:" . "她已经是我女神了，谁都别和我抢！" . "\n";
                                    } elseif ($beauty['female_score'] >= 90 && $beauty['female_score'] < 100) {
                                        $female_score= "女性认为颜值程度:" . "你已经美出了天际，无法用语言形容！凤姐都没你漂亮！" . "\n"
                                            . "男性认为颜值程度:" . "你已经帅出了天际，无法用语言形容！凤姐都没你漂亮！！" ;
                                    }
                                    $contentStr= "$value"."\n"."性别:" . "女" ."\n"."年龄:" . $age['value']."岁"."\n". "健康状态:" . $skinstatus['health']."\n" ."$female_score". "女性认为颜值程度:" . $beauty['female_score'] . "\n" . "男性认为颜值程度:" . $beauty['female_score']. "\n". "平静程度:" . $emotion['neutral'] . "\n". "高兴程度:" . $emotion['happiness'] . "\n";
                                    $resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
                                }
...
//                $resultStr = $this->transmitMusic($postObj, $content);
//            }
//        }else{
...
//        }
    ...
    public function downimg($media_id,$filename){//通过media_id下载素材到服务器,公众号可以使用本接口获取临时素材（即下载临时的多媒体文件）。请注意，视频文件不支持https下载，调用该接口需http协议。
        $access_token=$this->readtoken();
        $url="https://api.weixin.qq.com/cgi-bin/media/get?access_token=".$access_token."&media_id=".$media_id;
        $image=$this->downloadImageFromWeiXin($url);
...
//        M("text")->add($data);
        ...
        if ($timenow - $lasttime < 5000&&(!$is_refresh) && $token) {
            $tokenread = $token['token_value'];
//            $data['content'] = $tokenread;
//            $data['addtime'] = date("Y-m-d H:i:s",time());
            //M("text")->add($data);
        } else {
...
if ($christmas_prize[christmas_prize_type] == 3) {
    ...
    {
            //$domainName=C("domainName");
           $domainName= 'http://lll.lkws.shop';

   /*$data = '{"button":[{"name":"商城","sub_button":[
   {"type":"view","name":"商城首页","url":"'.$domainName.'/index.php/Home/Index/index"},
   {"type":"view","name":"摇一摇","url":"'.$domainName.'/index.php/Home/Lu/yaoyiyao.html"},
   ]},*/
...
{"type":"view","name":"商城首页","url":"'.$domainName.'/index.php/Home/Index/index"}
   ...
   {"name":"商品","sub_button":[
   {"type":"view","name":"找茬小游戏","url":"'.$domainName.'/index.php/Home/Lu/zhaocha.html"},
   {"type":"view","name":"摇一摇","url":"'.$domainName.'/index.php/Home/Lu/yaoyiyao.html"},
   {"type": "pic_sysphoto","name": "系统拍照识别","key": "rselfmenu_1_0","sub_button": [ ]},
   {"type":"view","name":"登录","url":"'.$domainName.'/index.php/Home/Lu/login.html"},
   {"type":"view","name":"注册","url":"'.$domainName.'/index.php/Home/Lu/register.html"}
   ]},
   ...
   {"type":"view","name":"面部识别","url":"'.$domainName.'/index.php/Home/Lu/face.html"},
   {"type":"click","name":"讲笑话","key":"笑话"},
   {"type":"view","name":"颜值检测","url":"'.$domainName.'/index.php/Home/Lu/face.html"},
   {
            "name": "发送位置",
     ...
     */
    public function getQrcodeTicket($scene_id,$is_id){
        if($is_id==1){
            $data ='{
             "expire_seconds":1800,
        ...
        $result=json_decode(https_request($url,$data));
        $ticket=$result->ticket;
        return $ticket;
    }
    public function geturl($scene_id,$is_id){//带参数二微码url
        $ticket=$this->getQrcodeTicket($scene_id,$is_id);//1 临时二微码 2 永久二微码
        $url="https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=".urlencode($ticket);//urlencode()函数原理就是首先把中文字符转换为十六进制，然后在每个字符前面加一个标识符%。urldecode()函数与urlencode()函数原理相反，用于解码已编码的 URL 字符串，其原理就是把十六进制字符串转换为中文字符
        return $url;
    }
